...
public abstract class BaseSearchProcessor {
     ...
     */
    public String generateQL(String entityName, ISearch search, List<Object> paramList) {
        if (entityName == null)
            throw new NullPointerException("The entity class for a search cannot be null");

        SearchContext ctx = new SearchContext(entityName, rootAlias, paramList);
     ...
     */
    public String generateRowCountQL(String entityName, ISearch search, List<Object> paramList) {
        if (entityName == null)
            throw new NullPointerException("The entity class for a search cannot be null");
    ...
    protected String generateFromClause(SearchContext ctx, boolean doEagerFetching) {
        ...
        sb.append(getMetadataUtil().get(ctx.entityName).getEntityName());
        sb.append(" ");
        sb.append(ctx.getRootAlias());
    ...
    protected String generateOrderByClause(SearchContext ctx, List<Sort> sorts) {
        ...
        for (Sort sort : sorts) {
            ...
            } else if (sort.isIgnoreCase() && metadataUtil.get(ctx.entityName, sort.getProperty()).isString()) {
                sb.append("lower(");
    ...
    protected String filterToQL(SearchContext ctx, Filter filter) {
        ...
        if (filter.isTakesListOfValues()) {
            value = prepareValue(ctx.entityName, property, value, true);
        } else if (filter.isTakesSingleValue()) {
            value = prepareValue(ctx.entityName, property, value, false);
        }
        ...
        switch (operator) {
            ...
            case Filter.OP_EMPTY:
                metadata = metadataUtil.get(ctx.entityName, property);
     ...
     * @param property - the property of the main query that points to the collection
     *                 on which to query
    ...
    protected String generateSubquery(SearchContext ctx, String property, Filter filter) {
        ...
        ctx2.entityName = metadataUtil.get(ctx.entityName, property).getEntityName();
     ...
     * @param operation - a string used to fill in the collection operation. The value
    ...
    protected String generateSimpleAllOrSome(SearchContext ctx, String property, Filter filter, String operation) {
        ...
        Object value = InternalUtil.convertIfNeeded(filter.getValue(), metadataUtil.get(ctx.entityName, property)
                .getJavaClass());
     ...
     * Long to and Integer.
     *
     * @param isCollection <code>true</code> if the value is a collection of values, for
     *                     example with IN and NOT_IN operators.
     * @return the converted value.
     */
    @SuppressWarnings("unchecked")
    protected Object prepareValue(String entityName, String property, Object value, boolean isCollection) {
        if (value == null)
            return null;
        ...
        if (property != null && ("class".equals(property) || property.endsWith(".class"))) {
            ...
            expectedClass = metadataUtil.get(entityName, property).getJavaClass();
        }
    ...
    protected String[] splitPath(SearchContext ctx, String path) {
        ...
        if (pos == -1) {
            ...
            while (true) {
                if (metadataUtil.isId(ctx.entityName, currentPath)) {
                ...
                } else if (!first && metadataUtil.get(ctx.entityName, currentPath).isEntity()) {
                ...
                if (pos != -1 && lastSegment.equals("size")
                        && metadataUtil.get(ctx.entityName, currentPath.substring(0, pos)).isCollection()) {
                    first = true;
                }
    ...
    protected static final class SearchContext {
        ...
        public SearchContext(String entityName, String rootAlias, List<Object> paramList) {
    ...
    public Filter getFilterFromExample(String entityName, Object example) {
        return getFilterFromExample(entityName, example, null);
    }
    ...
    public Filter getFilterFromExample(String entityName, Object example, ExampleOptions options) {
        ...
        Metadata metadata = metadataUtil.get(entityName);
